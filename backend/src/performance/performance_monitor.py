"""
Performance Monitor - Phase 3: Additional Optimizations
...
class PerformanceMonitor:
    ...
    def _monitor_loop(self):
        ...
        while self.running:
            try:
                ...
                asyncio_tasks = 0
                try:
                    # Get event loop if available
                    # Note: This is a simplified approach - in production you'd want to track the specific event loop
                    asyncio_tasks = 0  # Will be updated by app-specific code
                except:
                    pass
                
                # Application-specific metrics (will be set by external code)
                ...
                # Log periodic status (every 10 samples = 5 minutes with 30s interval)
                if len(self.metrics['timestamps']) % 10 == 0:
                    logger.info(
                        f"📊 System Status: CPU={cpu_percent:.1f}%, "
                        f"RAM={memory_mb:.1f}MB ({memory_percent:.1f}%), "
                        f"Tasks={asyncio_tasks} async/{managed_tasks} managed, "
                        f"Traces={trace_count}, Files={open_files}, Threads={thread_count}"
                    )
    ...
    def get_current_stats(self) -> Dict[str, Any]:
        ...
        latest_idx = -1
        return {
            'cpu_percent': self.metrics['cpu_percent'][latest_idx] if self.metrics['cpu_percent'] else 0,
            'memory_mb': self.metrics['memory_mb'][latest_idx] if self.metrics['memory_mb'] else 0,
            'memory_percent': self.metrics['memory_percent'][latest_idx] if self.metrics['memory_percent'] else 0,
            'open_files': self.metrics['open_files'][latest_idx] if self.metrics['open_files'] else 0,
            'thread_count': self.metrics['thread_count'][latest_idx] if self.metrics['thread_count'] else 0,
            'asyncio_tasks': self.metrics['asyncio_tasks'][latest_idx] if self.metrics['asyncio_tasks'] else 0,
            'kafka_consumer_status': self.metrics['kafka_consumer_status'][latest_idx] if self.metrics['kafka_consumer_status'] else "unknown",
            'trace_count': self.metrics['trace_count'][latest_idx] if self.metrics['trace_count'] else 0,
            'active_managed_tasks': self.metrics['active_managed_tasks'][latest_idx] if self.metrics['active_managed_tasks'] else 0,
            'timestamp': datetime.fromtimestamp(self.metrics['timestamps'][latest_idx]).isoformat() if self.metrics['timestamps'] else None,
    ...
    def get_trends(self, samples: int = 10) -> Dict[str, Any]:
        ...
        sample_count = min(samples, len(self.metrics['timestamps']))
        
        return {
            'cpu_trend': calculate_trend(self.metrics['cpu_percent'], sample_count),
            'memory_trend_mb': calculate_trend(self.metrics['memory_mb'], sample_count),
            'tasks_trend': calculate_trend(self.metrics['asyncio_tasks'], sample_count),
            'trace_trend': calculate_trend(self.metrics['trace_count'], sample_count),
            'sample_count': sample_count,
            'time_span_minutes': (
                (self.metrics['timestamps'][-1] - self.metrics['timestamps'][-sample_count]) / 60
                if len(self.metrics['timestamps']) >= sample_count else 0
            ),
            'recent_alerts': len([a for a in self.alerts_triggered if time.time() - a['timestamp'] < 3600])  # Last hour
        }
    ...
    def get_full_history(self) -> Dict[str, List]:
        ...
        return {
            ...
            if values  # Only return non-empty metrics
        }
