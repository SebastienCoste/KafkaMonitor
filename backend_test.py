...
def run_review_request_tests():
    ...
    def test_deployment_endpoints_405_fix(self) -> bool:
        """Test the fixed deployment endpoints to verify 405 errors are resolved - REQ FIX2"""
        print("\n" + "=" * 80)
        print("🔧 Testing Fixed Deployment Endpoints - REQ FIX2")
        print("=" * 80)
        print("Testing POST /api/blueprint/validate/{filename} and POST /api/blueprint/activate/{filename}")
        print("with corrected request payload including tgz_file, environment, and action fields")
        
        try:
            # Step 1: Set root path to /app as requested
            print("\n📁 Step 1: Setting root path to /app...")
            ...
            if config_response.status_code == 200:
                self.log_test("Set Root Path to /app", True, f"Root path configured successfully")
            else:
                self.log_test("Set Root Path to /app", False, f"Failed to set root path: HTTP {config_response.status_code}")
                return False
            
            # Step 2: Create a test file for deployment testing
            print("\n📄 Step 2: Creating test file for deployment testing...")
            test_filename = "test-deployment.tgz"
            
            # Step 3: Test POST /api/blueprint/validate/{filename} with corrected payload
            print(f"\n🔍 Step 3: Testing POST /api/blueprint/validate/{test_filename}...")
            validate_payload = {
                "tgz_file": test_filename,
                "environment": "DEV", 
                "action": "validate"
            }
            
            print(f"   Request payload: {validate_payload}")
            ...
            print(f"   Response: HTTP {validate_response.status_code}")
            
            # Verify no 405 Method Not Allowed error
            if validate_response.status_code == 405:
                self.log_test("Blueprint Validate Endpoint 405 Fix", False, "Still returning 405 Method Not Allowed")
                return False
            elif validate_response.status_code in [200, 400, 500]:
                self.log_test("Blueprint Validate Endpoint 405 Fix", True, f"✅ No 405 error - HTTP {validate_response.status_code}")
                ...
                try:
                    ...
                    print(f"   Response text: {validate_response.text[:200]}...")
            else:
                self.log_test("Blueprint Validate Endpoint 405 Fix", False, f"Unexpected status: {validate_response.status_code}")
                return False
            
            # Step 4: Test POST /api/blueprint/activate/{filename} with corrected payload
            print(f"\n🚀 Step 4: Testing POST /api/blueprint/activate/{test_filename}...")
            activate_payload = {
                "tgz_file": test_filename,
                "environment": "DEV",
                "action": "activate"
            ...
            print(f"   Request payload: {activate_payload}")
            ...
            print(f"   Response: HTTP {activate_response.status_code}")
            
            # Verify no 405 Method Not Allowed error
            if activate_response.status_code == 405:
                self.log_test("Blueprint Activate Endpoint 405 Fix", False, "Still returning 405 Method Not Allowed")
                return False
            elif activate_response.status_code in [200, 400, 500]:
                self.log_test("Blueprint Activate Endpoint 405 Fix", True, f"✅ No 405 error - HTTP {activate_response.status_code}")
                ...
                try:
                    ...
                    print(f"   Response text: {activate_response.text[:200]}...")
            else:
                self.log_test("Blueprint Activate Endpoint 405 Fix", False, f"Unexpected status: {activate_response.status_code}")
                return False
            
            # Step 5: Test script endpoints as well
            print(f"\n📜 Step 5: Testing script endpoints...")
            
            # Test POST /api/blueprint/validate-script/{filename}
            print(f"   Testing POST /api/blueprint/validate-script/{test_filename}...")
            validate_script_payload = {
                "tgz_file": test_filename,
                "environment": "DEV",
                "action": "validate"
            ...
            print(f"   Response: HTTP {validate_script_response.status_code}")
            
            if validate_script_response.status_code == 405:
                self.log_test("Blueprint Validate Script Endpoint 405 Fix", False, "Still returning 405 Method Not Allowed")
                return False
            elif validate_script_response.status_code in [200, 400, 404, 500]:
                self.log_test("Blueprint Validate Script Endpoint 405 Fix", True, f"✅ No 405 error - HTTP {validate_script_response.status_code}")
            else:
                self.log_test("Blueprint Validate Script Endpoint 405 Fix", False, f"Unexpected status: {validate_script_response.status_code}")
                return False
            
            # Test POST /api/blueprint/activate-script/{filename}
            print(f"   Testing POST /api/blueprint/activate-script/{test_filename}...")
            activate_script_payload = {
                "tgz_file": test_filename,
                "environment": "DEV",
                "action": "activate"
            ...
            print(f"   Response: HTTP {activate_script_response.status_code}")
            
            if activate_script_response.status_code == 405:
                self.log_test("Blueprint Activate Script Endpoint 405 Fix", False, "Still returning 405 Method Not Allowed")
                return False
            elif activate_script_response.status_code in [200, 400, 404, 500]:
                self.log_test("Blueprint Activate Script Endpoint 405 Fix", True, f"✅ No 405 error - HTTP {activate_script_response.status_code}")
            else:
                self.log_test("Blueprint Activate Script Endpoint 405 Fix", False, f"Unexpected status: {activate_script_response.status_code}")
                return False
            
            print("\n✅ All deployment endpoints accept POST requests correctly!")
            ...
            self.log_test("Deployment Endpoints 405 Fix", False, f"Exception: {str(e)}")
